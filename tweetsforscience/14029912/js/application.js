...
//
  ...
  exports.init = function () {
    ...
    $('.navbar-search').submit(function(e) {
      ...
      var whiteSpaceRegEx = /^\s+|\s+$/g,
          searchInput = $(this).find('.search-query').val(),
          searchString = searchInput.replace(whiteSpaceRegEx, '');
  ...
  (function(exports) {
    exports.base = {
      ...
      buildNavigation: function () {
        ...
        for (var i=0; i < this.status_index.length; i++) {
          var title_str = months[this.status_index[i]['month'] - 1] + ' ' +
                          this.status_index[i]['year'];
          var month_index = {
                this_year: this.status_index[i]['year'],
                this_month: this.status_index[i]['month'],
                this_count: Grailbird.insertCommas(this.status_index[i]['tweet_count']),
                this_height: (this.status_index[i]['tweet_count'] / month_count_max['tweet_count']) * 100,
                str_title: title_str,
                str_count: Grailbird.insertCommas(this.status_index[i]['tweet_count']) +
                    ' ' + this.pluralize(this.status_index[i]['tweet_count']),
                data_idx: i
              };

          this.status_index[i]['title_str'] = title_str;

          if( this.status_index[i]['year'] !== year_curr ){
            renderYear();
      ...
      displayTweets: function (tweet_index_id) {
        ...
        var self = this,
            ...
            title = templates.header_str.render({
              ...
              count: Grailbird.insertCommas(tweet_month['tweet_count']),
              content_type: this.pluralize(tweet_month['tweet_count'])
            }),
            showTweets = function() {
              $('.tweets-header-title').fadeOut(100);
              $('.container .contents .tweets').fadeOut(100, function () {
                $('.tweets-header-title').empty().html(title).attr('title', '');
                $(this).empty();
                $(window).scrollTop(0);
        ...
        if(this.status_index[tweet_index_id].year === undefined) {
  ...
  exports.loadScript = function (tweet_month, callback) {
    var fileName = tweet_month['file_name'],
